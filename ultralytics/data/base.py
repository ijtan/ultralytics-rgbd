...
class BaseDataset(Dataset):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    ...
                    im = cv2.imread(f, cv2.IMREAD_UNCHANGED)  # RGB + Depth
            else:  # read image
    ...
    def cache_images_to_disk(self, i):
        ...
        if not f.exists():
            np.save(
                f.as_posix(),
                cv2.imread(self.im_files[i], cv2.IMREAD_UNCHANGED),
                allow_pickle=False,
            )
    ...
    def check_cache_ram(self, safety_margin=0.5):
        ...
        for _ in range(n):
            im = cv2.imread(random.choice(self.im_files), cv2.IMREAD_UNCHANGED)
